...
from palsav_lite.archive import FArchiveReader, FArchiveWriter
from palsav_lite.rawdata.common import pal_item_and_num_read, pal_item_and_slot_writer
...
def encode_bytes(p: dict[str, Any], module_type: str) -> bytes:
    ...
    if 'trailing_bytes' in p:
        if module_type != 'EPalBaseCampModuleType::TransportItemDirector':
            writer.write(bytes(p['trailing_bytes']))
